...
export function AppSidebar({ collapsed = false, onCollapse }: AppSidebarProps) {
  ...
  const NavLink = ({
    ...
    active,
    showText = true,
  }: { item: (typeof generalNav)[0]; active: boolean; showText?: boolean }) => (
    <Link
      href={item.href}
  ...
  return (
    <>
        ...
        {mobileOpen && (
          ...
          >
            ...
            <div className="p-3 border-t border-sidebar-border space-y-1 shrink-0">
              ...
              <p className="px-3 pt-2 text-[10px] text-muted-foreground">devlens v0.1.0</p>
            </div>
          </motion.aside>
        )}
      </AnimatePresence>
      ...
      >
          ...
          <AnimatePresence mode="wait">
            {!collapsed && (
              <motion.div
                ...
                transition={{ duration: 0.15 }}
              >
                <Link href="/dashboard" className="flex items-center gap-1 whitespace-nowrap">
                  <span className="brand-text">Dev</span>
                  <span className="brand-text brand-text-accent">Lens</span>
                </Link>
              </motion.div>
        ...
        {/* Navigation */}
        <nav className={cn("flex-1 overflow-y-auto py-4 space-y-6 overflow-x-hidden", collapsed ? "px-2" : "px-3")}>
          <div>
            {!collapsed && (
            ...
            <div className="space-y-1">
              {generalNav.map((item) => (
                <NavLink key={item.name} item={item} active={pathname === item.href} showText={!collapsed} />
              ))}
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="space-y-1">
              {feedbackNav.map((item) => (
                <NavLink key={item.name} item={item} active={false} showText={!collapsed} />
              ))}
            </div>
          </div>
          <div>
        ...
        {/* Footer */}
        <div
          className={cn("p-3 border-t border-sidebar-border space-y-1 shrink-0 overflow-hidden", collapsed && "px-2")}
        >
          <button
            onClick={() => setTheme(resolvedTheme === "dark" ? "light" : "dark")}
            className={cn(
          ...
          {!collapsed && (
            <p className="px-3 pt-2 text-[10px] text-muted-foreground whitespace-nowrap">devlens v0.1.0</p>
          )}
        </div>
      </aside>
    </>
...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main
        className={cn("min-h-screen transition-all duration-300 pt-14 lg:pt-0", collapsed ? "lg:ml-16" : "lg:ml-56")}
      >
        {children}
      </main>
    </div>
